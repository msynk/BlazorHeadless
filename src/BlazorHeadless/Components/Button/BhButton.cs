...
///     set <see cref="BhComponentBase.As"/> to "a", "div", etc.
...
///     <see cref="BhButtonContext"/> enabling conditional UI (spinners, icons)
...
/// <para><b>Usage:</b></para>
/// <code>
/// &lt;BhButton OnClick="Save" Loading="@isSaving" Context="btn"&gt;
///     @if (btn.Loading) { &lt;span&gt;Saving…&lt;/span&gt; }
///     else              { &lt;span&gt;Save&lt;/span&gt; }
/// &lt;/BhButton&gt;
/// </code>
/// </summary>
public class BhButton : BhComponentBase
{
    ...
    /// <summary>HTML button type attribute. Ignored when <see cref="BhComponentBase.As"/> is not "button" or "input".</summary>
    [Parameter]
    ...
    /// <summary>
    /// Content template receiving <see cref="BhButtonContext"/> for state-driven rendering.
    /// Plain content (without referencing context) works equally well.
    /// </summary>
    [Parameter]
    public RenderFragment<BhButtonContext>? ChildContent { get; set; }

    protected override string DefaultTag => "button";

    private BhButtonContext Context => new()
    {
        Disabled = Disabled,
        Loading = Loading,
        Type = Type
