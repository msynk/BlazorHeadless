...
/// <summary>
/// The interactive trigger button for an <see cref="BhAccordionItem"/>. Clicking it
/// toggles the associated <see cref="BhAccordionContent"/> open or closed.
///
...
///     <see cref="BhAccordionTriggerContext"/> so consumers can render a rotating chevron
...
/// </code>
/// </summary>
public class BhAccordionTrigger : BhComponentBase
{
    [CascadingParameter]
    private BhAccordionItemContext ItemContext { get; set; } = default!;

    /// <summary>
    /// Content template receiving <see cref="BhAccordionTriggerContext"/> for state-driven rendering.
    /// Plain content (without referencing context) works equally well.
    /// </summary>
    [Parameter]
    public RenderFragment<BhAccordionTriggerContext>? ChildContent { get; set; }

    protected override string DefaultTag => "button";

    private BhAccordionTriggerContext TriggerContext => new()
    {
