...
/// </para>
/// </summary>
public class BhComboboxButton<TValue> : BhComponentBase
{
    [CascadingParameter]
    private BhComboboxContext<TValue> BhComboboxContext { get; set; } = default!;

    /// <summary>Content template receiving <see cref="BhComboboxButtonRenderContext{TValue}"/>.</summary>
    [Parameter]
    public RenderFragment<BhComboboxButtonRenderContext<TValue>>? ChildContent { get; set; }

    protected override string DefaultTag => "button";

    private BhComboboxButtonRenderContext<TValue> RenderContext => new()
    {
        IsOpen = BhComboboxContext?.IsOpen ?? false,
        Disabled = BhComboboxContext?.Disabled ?? false,
    };
    ...
    protected override Dictionary<string, object> BuildComponentAttributes()
    {
        ...
        var isDisabled = BhComboboxContext?.Disabled ?? false;
    ...
    {
        return BhComboboxContext?.ToggleAsync() ?? Task.CompletedTask;
    }
}
