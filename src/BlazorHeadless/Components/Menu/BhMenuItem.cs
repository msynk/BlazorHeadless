...
/// <summary>
/// A single item inside an <see cref="BhMenuItems"/>. Renders as
...
///   <item>Render-prop via <see cref="ChildContent"/> exposes <see cref="BhMenuItemRenderContext"/>.</item>
/// </list>
///
/// <para>
...
/// </para>
/// </summary>
public class BhMenuItem : BhComponentBase, IDisposable
{
    [CascadingParameter]
    ...
    /// <summary>Content template receiving <see cref="BhMenuItemRenderContext"/> for state-driven rendering.</summary>
    [Parameter]
    public RenderFragment<BhMenuItemRenderContext>? ChildContent { get; set; }
    ...
    public bool IsActive => BhMenuContext?.ActiveIndex == _index;

    /// <summary>Whether this item is disabled (own flag or root menu).</summary>
    public bool IsItemDisabled => Disabled || (BhMenuContext?.Disabled ?? false);

    private BhMenuItemRenderContext RenderContext => new()
    {
        IsActive = IsActive,
    ...
    protected override void OnInitialized()
    {
        if (BhMenuContext is not null)
            _index = BhMenuContext.RegisterItem(this);
    }

    public void Dispose()
    {
        BhMenuContext?.UnregisterItem(this);
    }
    ...
    protected override void BuildRenderTree(RenderTreeBuilder builder)
    {
        ...
        builder.AddAttribute(10, "id", BhMenuContext is not null ? BhMenuContext.GetItemId(_index) : ComponentId);
    ...
    private Task HandleClick(MouseEventArgs _)
    {
        ...
        return BhMenuContext?.ActivateItemAsync(_index) ?? Task.CompletedTask;
    }

    private void HandleMouseEnter(MouseEventArgs _)
    {
        if (IsItemDisabled) return;
        BhMenuContext?.SetActiveIndex(_index);
    }
}
