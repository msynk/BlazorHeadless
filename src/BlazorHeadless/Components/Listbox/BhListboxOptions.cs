...
/// <c>&lt;ul role="listbox"&gt;</c> by default (override via <see cref="BhComponentBase.As"/>).
///
/// <para>
...
/// relative to the <see cref="BhListboxButton{TValue}"/> using the anchor positioning system.
/// </para>
/// </summary>
public class BhListboxOptions<TValue> : BhComponentBase, IAsyncDisposable
{
    ...
    /// <summary>Child content. Should contain one or more <see cref="BhListboxOption{TValue}"/> components.</summary>
    [Parameter]
    public RenderFragment? ChildContent { get; set; }

    /// <summary>
    /// Configures automatic positioning of the dropdown relative to the
    /// <see cref="BhListboxButton{TValue}"/>. When set, the panel is positioned using
    ...
    private bool IsOpen => BhListboxContext?.IsOpen ?? false;

    protected override void BuildRenderTree(RenderTreeBuilder builder)
    {
        builder.OpenElement(0, Tag);
        builder.AddAttribute(10, "id", BhListboxContext?.OptionsId ?? ComponentId);
    ...
    protected override Dictionary<string, object> BuildComponentAttributes()
    {
        ...
        if (BhListboxContext?.Multiple == true)
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        if (IsOpen && !_wasOpen)
        {
            var buttonId = BhListboxContext.ButtonId;
            var panelId = BhListboxContext.OptionsId;
